...
tb_pct_cursos_pivot AS (
    SELECT 
        ...
        sum(CASE WHEN descSlugCurso = 'descSlugCurso' THEN pctCursoCompleto ELSE 0 END) AS descSlugCurso,
...
tb_ultima_atividade AS (
    ...
    SELECT
        idUsuario, 
<<<<<<< HEAD
        min(julianday('{date}') - julianday(dtCriacao)) AS qtdeDiasUltAtividades
=======
        min(julianday('2025-10-01') - julianday(dtCriacao)) AS qtdeDiasUltAtividades
>>>>>>> reorganizacao-local
